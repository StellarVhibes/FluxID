...
export default function ExplanationCard({ explanation, className = "" }: Props) {
  ...
  const badgeBg = isLlm ? "var(--primary)" : "var(--surface)";
  const badgeFg = isLlm ? "var(--background)" : "var(--foreground-muted)";
  const badgeLabel = isLlm ? "AI" : "RULES";
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
        <div className="flex items-center gap-2">
          ...
          <span
            ...
            title={
              isLlm
                ? `Generated by ${explanation.model ?? "LLM"} — content varies, not deterministic`
                : "Deterministic rule-based explanation — same inputs always produce the same output"
            }
          >
            {badgeLabel}
          </span>
